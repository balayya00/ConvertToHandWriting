"""
PDF to Handwriting Converter – app.py v5.0
"""
...
# ── Start background font downloads immediately at import time ──
try:
    from utils.font_manager import FontManager
    _fm = FontManager(FONTS_DIR)
    _fm.start_background_download()
    log.info("Background font download started at app load.")
except Exception as _e:
    log.warning(f"Font manager init warning: {_e}")
...
@app.route("/health")
def health():
    from utils.font_manager import FONTS, FONTS_DIR as FD
    cached = sum(1 for v in FONTS.values() if (FONTS_DIR / v["file"]).exists())
    return jsonify(status="ok", fonts_cached=f"{cached}/{len(FONTS)}")
...
def api_extract():
    try:
        ...
        if not _ok(f.filename):
            return jsonify(error="Upload PDF, JPG, or PNG only"), 400

        ext = _ext(f.filename)
        tmp = UPLOAD_DIR / f"{_sid()}.{ext}"
        f.save(str(tmp))
...
def api_convert():
    try:
        ...
        log.info(
            f"Convert: font={cfg['font_name']} fmt={cfg['output_format']} "
            f"pages={len(pages)} paper={cfg['paper_style']}"
        )
        ...
        if fmt in ("jpg", "both"):
            ...
            out.setdefault("preview_url",
                           out["jpg_urls"][0] if out["jpg_urls"] else "")

        log.info(f"Convert done: {sid}")
        return jsonify(out)
